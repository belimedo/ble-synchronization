...
#define THRESHOLD_VALUE_DEFAULT (65300) // 0xFF01
#define SQUARE_ROOT_2           (1.41421)
...
void calculatePowerFromArray(bool reconstructionCall)
{
    ...
    uint16_t combinedCurrentBuffer[combinedElements] = {0};
    uint16_t combinedVoltageBuffer[combinedElements] = {0};
    if(xSemaphoreTake(receiveBufferMutex[0], 5) == pdTRUE)
    {
        memcpy(combinedCurrentBuffer, &readDataBuffers[0].currentBuffer[firstBufferSkip], sizeof(uint16_t) * (SEND_BUFFER_ELEMENTS - firstBufferSkip));
        memcpy(combinedVoltageBuffer, &readDataBuffers[0].voltageBuffer[firstBufferSkip], sizeof(uint16_t) * (SEND_BUFFER_ELEMENTS - firstBufferSkip));
    ...
    {
        memcpy(&combinedCurrentBuffer[(SEND_BUFFER_ELEMENTS - firstBufferSkip)], &readDataBuffers[1].currentBuffer[secondBufferSkip], sizeof(uint16_t) * (SEND_BUFFER_ELEMENTS - secondBufferSkip));
        memcpy(&combinedVoltageBuffer[(SEND_BUFFER_ELEMENTS - firstBufferSkip)], &readDataBuffers[1].voltageBuffer[secondBufferSkip], sizeof(uint16_t) * (SEND_BUFFER_ELEMENTS - secondBufferSkip));
    ...
    else
    {
        ...
        {
            uint16_t maxVoltage = combinedCurrentBuffer[periodCounter];
            uint16_t maxCurrent = combinedVoltageBuffer[periodCounter];
