...
#define THRESHOLD_VALUE         (65555) // 0xFF01
...
void fillFutureBuffer(void* arg)
{
    if(xSemaphoreTake(storeBufferMutex[alertStoreBufferIdx], 2) == pdTRUE) // busy wait 5 ms
    {
        if (DEBUG_PRINTS)
        {
            Serial.printf("[Future buffer] Alert detected at %d idx, current value:0x%4x, voltage value:0x%4x\n", 
        ...
        if (endBufferIdx > STORE_BUFFER_ELEMENTS)
        {
            ...
            if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], 2) == pdTRUE)
            {
                ...
                memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[alertStoreBufferIdx].voltageBuffer[alertEventIdx], firstBufferSize);
        ...
        else
        {
            ...
            if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], 2) == pdTRUE)
            {
                ...
                memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[alertStoreBufferIdx].voltageBuffer[alertEventIdx], SEND_BUFFER_SIZE);
                xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
            }
            else
            {
...
void sampleData(void* arg) {
    ...
    if(xSemaphoreTake(storeBufferMutex[currentStoreBufferIdx], 2) == pdTRUE)  // Minimalno blocking vrijeme mora biti manje od (1/SAMPLING_FREQUENCY)/2 10 ms / 2 je 5
    {
        ...
        electricCurrentData = ELECTRIC_CURRENT_DATA[sampleCounter % ELECTRIC_CURRENT_DATA_SIZE]; // This can be read from ADC
        voltageData = VOLTAGE_DATA[sampleCounter % VOLTAGE_DATA_SIZE]; // This can be read from ADC
        ...
        if (sendAlerts && (electricCurrentData >= THRESHOLD_VALUE || voltageData >= THRESHOLD_VALUE)) // lazy evaluation
        {
            if (DEBUG_PRINTS)
            {
        ...
        { 
            ...
            if(xSemaphoreTake(storeBufferMutex[alertStoreBufferIdx], 2) == pdTRUE) // busy wait 5 ms
            {
                ...
                if (!alertDetected)
                {
                    ...
                    if (alertTimeDiff < 0)
                    {
                        Serial.println("[Data request] Something went wrong with alert time calculation!");
                    }
                ...
                if (sampleIdxDiff >= 0)
                {
                    if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], 2) == pdTRUE)
                    {
                        ...
                        memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[alertStoreBufferIdx].voltageBuffer[sampleIdxDiff], SEND_BUFFER_SIZE);
                        xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if(xSemaphoreTake(storeBufferMutex[prevAlertStoreBufferIdx], 2) == pdTRUE)
                    {
                        if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], 2) == pdTRUE)
                        {
                            ...
                            memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[prevAlertStoreBufferIdx].voltageBuffer[prevStartIdx], prevSize);
                            xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
                        }
                        else
                        {
...
//             esp_timer_start_periodic(timestampTimer, TIMESYNC_INTERVAL_US); // Nece ga pokrenuti odmah, zato treba da odradimo prvo ovaj start_once() da se zabiljezi da je sve gotovo.
...
void setup() {
    ...
    Serial.printf("Started sampling timer with sampling interval: %d - %f\n", SAMPLE_INTERVAL_US, samplingIntervalUs);
