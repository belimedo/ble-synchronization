...
SemaphoreHandle_t readBufferMutex[NUMBER_OF_SEND_BUFFERS];
...
void printPowerValues()
{
    for (int sendingBufferIdx = 0; sendingBufferIdx < NUMBER_OF_SEND_BUFFERS; sendingBufferIdx++)
    {
        ...
        if(xSemaphoreTake(readBufferMutex[sendingBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE)
        {
            ...
            xSemaphoreGive(readBufferMutex[sendingBufferIdx]);
        }
        else
        {
...
void calculatePowerFromArray(bool reconstructionCall)
{
    ...
    if ((xSemaphoreTake(printBufferMutex, pdMS_TO_TICKS(1)) == pdTRUE))
    {
        int16_t combinedCurrentBuffer[combinedElements] = {0};
        int16_t combinedVoltageBuffer[combinedElements] = {0};
        if((xSemaphoreTake(readBufferMutex[0], pdMS_TO_TICKS(1)) == pdTRUE))
        {
            ...
            xSemaphoreGive(readBufferMutex[0]);
        }
        else
        {
            Serial.println("[Calculate power] Unable to fetch past buffer's mutex.");
        }
        if(xSemaphoreTake(readBufferMutex[1], pdMS_TO_TICKS(1)) == pdTRUE)
        {
            ...
            xSemaphoreGive(readBufferMutex[1]);
        }
        else
        {
...
void dataTransferNotifyCallback(BLERemoteCharacteristic* pBLERemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify) 
{
    ...
    if(xSemaphoreTake(receiveBufferMutex, pdMS_TO_TICKS(1)) == pdTRUE)
    {
        ...
        {
            if(xSemaphoreTake(readBufferMutex[buffersReceived], pdMS_TO_TICKS(1)) == pdTRUE)
            {
                ...
                sendCommand = true;
                xSemaphoreGive(readBufferMutex[buffersReceived]);
    ...
    {
        Serial.println("[Data transfer] Can't get recieve buffer mutex.");
    }
...
void setup() {
    ...
    {
        // kreiraj mutekse
        readBufferMutex[i] = xSemaphoreCreateMutex();
        if(readBufferMutex[i] == NULL) {
