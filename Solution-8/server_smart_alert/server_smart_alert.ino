...
void sampleData(void* arg) {
    ...
    if(xSemaphoreTake(storeBufferMutex[currentStoreBufferIdx], 0) == pdTRUE) 
    {
        ...
        // if (sendAlerts && deviceConnected && sampledData >= THRESHOLD_VALUE && (xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], 10) == pdTRUE)) // TODO: Mozda bih mogao samo sendAlerts onemoguciti dok se ne zavrsi proces
