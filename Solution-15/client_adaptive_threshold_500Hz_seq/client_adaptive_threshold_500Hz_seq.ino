...
void calculatePowerFromArray(bool reconstructionCall)
{
    ...
    if ((xSemaphoreTake(powerCalculationMutex, pdMS_TO_TICKS(1)) == pdTRUE))
    {
        ...
        for (int serverIdx = 0; serverIdx < NUMBER_OF_SERVERS; serverIdx++)
        {
            ...
            {
                memcpy(&combinedCurrentBuffer[serverIdx][(SEND_BUFFER_ELEMENTS - firstBufferSkip)], &transferDataBuffers[serverIdx][1].currentBuffer[1], sizeof(int16_t) * (SEND_BUFFER_ELEMENTS - secondBufferSkip));
                memcpy(&combinedVoltageBuffer[serverIdx][(SEND_BUFFER_ELEMENTS - firstBufferSkip)], &transferDataBuffers[serverIdx][1].voltageBuffer[1], sizeof(int16_t) * (SEND_BUFFER_ELEMENTS - secondBufferSkip));
        ...
        if (reconstructionCall)
        {
            ...
            for (int serverIdx = 0; serverIdx < NUMBER_OF_SERVERS; serverIdx++)
            {
                int16_t maxCurrent = combinedCurrentBuffer[serverIdx][alertPeriodIdx];
                int16_t maxVoltage = combinedVoltageBuffer[serverIdx][alertPeriodIdx];
                ...
                for (int i = alertPeriodIdx + 1; i < (alertPeriod + 1) * PERIOD_ELEMENTS; i++)
                {
                    ...
                    {
                        maxVoltage = combinedVoltageBuffer[serverIdx][i];
                        maxVoltageIdx = i;
                    }
                    if (combinedCurrentBuffer[serverIdx][i] >= maxCurrent)
                    {
                        maxCurrent = combinedCurrentBuffer[serverIdx][i];
                        maxCurrentIdx = i;
                    }
                }
                currentThreshold[serverIdx] = maxCurrent > 0 ? (int16_t)ceil(maxCurrent * 1.1) : DEFAULT_THRESHOLD_CURRENT; // 10% increment of the threshold
                voltageThreshold[serverIdx] = maxVoltage > 0 ? (int16_t)ceil(maxVoltage * 1.1) : DEFAULT_THRESHOLD_VOLTAGE;
            ...
            {
                Serial.printf("[Reconstruction] %3d | %4d[V] %4d [A] | %4d[V] %4d[A] | %4d[V] %4d[A] |\n", 
        ...
        else
        {
            ...
            for (int periodCounter = 0; periodCounter < combinedElements; periodCounter+= PERIOD_ELEMENTS)
            {
                for (int serverIdx = 0; serverIdx < NUMBER_OF_SERVERS; serverIdx++)
                {
                    int16_t maxCurrent = combinedCurrentBuffer[serverIdx][periodCounter];
                    int16_t maxVoltage = combinedVoltageBuffer[serverIdx][periodCounter];
                    ...
                    for (int i = 1; i < PERIOD_ELEMENTS; i++)
                    {
                        ...
                        {
                            maxVoltage = combinedVoltageBuffer[serverIdx][periodCounter + i];
                            maxVoltageIdx = periodCounter + i;
                        }
                        if (combinedCurrentBuffer[serverIdx][periodCounter + i] > maxCurrent)
                        {
                            maxCurrent = combinedCurrentBuffer[serverIdx][periodCounter + i];
            ...
            // for (int serverIdx = 0; serverIdx < NUMBER_OF_SERVERS; serverIdx++)
            // {
            //     voltageThreshold[serverIdx] = DEFAULT_THRESHOLD_VOLTAGE;
            //     currentThreshold[serverIdx] = DEFAULT_THRESHOLD_CURRENT;
            //     thresholdsReady[serverIdx] = true;
            // }
        }
...
void processTxData(int serverIdx, uint8_t* data, size_t length) 
{
    Serial.printf("[Server %d TxData] Processing data.\n", serverIdx);
    ...
    {
        Serial.printf("[Server %d TxData] Unable to take mutex for data transfer.\n", serverIdx);
    }
}
...
void loop() {
    ...
    if (servers[0].connected && servers[1].connected && servers[2].connected)
    {
        ...
        if (allComplete) 
        {
            Serial.printf("[Loop] Transfers completed.\n");
            for (int i = 0; i < NUMBER_OF_SERVERS; i++) 
            {
