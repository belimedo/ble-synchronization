...
void printHexValues()
{
    ...
    for (int buffIdx = 0; buffIdx < NUMBER_OF_SEND_BUFFERS; buffIdx++)
    {
        Serial.printf("Printing %s buffer.", buffIdx % 2 == 0 ? "past" : "future");
        Serial.printf("Start time of the data: %llu\n Data samples:",readDataBuffers[buffIdx].timeStamp);
        for (int i = 0; i < SEND_BUFFER_SIZE; i++)
        {
            ...
            Serial.printf("%d: 0x%2x, ", i, readDataBuffers[buffIdx].dataBufferPtr[i]);
        }
        Serial.printf("\n");
    }
}
                           ...
                           uint8_t* pData, size_t length, bool isNotify) 
{
    ...
    else if((receivedBytes >= sizeof(sendBuffer)))
    {
        ...
        {
            Serial.printf("Past buffer transfer complete! Buffers received: %d/%d\n", buffersReceived + 1, NUMBER_OF_SEND_BUFFERS);
        ...
        {
            Serial.printf("Transfer complete! Buffers received: %d/%d\n", buffersReceived + 1, NUMBER_OF_SEND_BUFFERS);
            // Send transfer complete command
            request.command = 4;
