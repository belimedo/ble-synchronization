...
#include "esp_timer.h"
#include "../power_signal_data_long_alert.h"
    ...
    if(xSemaphoreTake(storeBufferMutex[alertStoreBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE) // busy wait 5 ms
    {
        ...
        if (endBufferIdx > STORE_BUFFER_ELEMENTS)
        {
            ...
            if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE)
            {
                ...
                memcpy(buffersToSend[currentSendingBufferIdx].currentBuffer, &storeBuffers[alertStoreBufferIdx].currentBuffer[alertEventIdx], sizeof(int16_t) * firstBufferSize);
                memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[alertStoreBufferIdx].voltageBuffer[alertEventIdx], sizeof(int16_t) * firstBufferSize);
            ...
            if(xSemaphoreTake(storeBufferMutex[nextAlertStoreBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE) // busy wait 50 ms
            {
                ...
                if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE)
                {
                    memcpy(buffersToSend[currentSendingBufferIdx].currentBuffer, &storeBuffers[nextAlertStoreBufferIdx].currentBuffer[0], sizeof(int16_t) * overheadIndices);
                    memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[nextAlertStoreBufferIdx].voltageBuffer[0], sizeof(int16_t) * overheadIndices);
                    xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
                }
                else
                {
        ...
        else
        {
            ...
            {
                buffersToSend[currentSendingBufferIdx].timeStamp = alertTimeStamp;
                memcpy(buffersToSend[currentSendingBufferIdx].currentBuffer, &storeBuffers[alertStoreBufferIdx].currentBuffer[alertEventIdx], sizeof(int16_t) * SEND_BUFFER_SIZE);
                memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[alertStoreBufferIdx].voltageBuffer[alertEventIdx], sizeof(int16_t) * SEND_BUFFER_SIZE);
                xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
            }
            else
            {
            ...
            if(xSemaphoreTake(storeBufferMutex[alertStoreBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE) // busy wait 5 ms
            {
                ...
                if (sampleIdxDiff >= 0)
                {
                    if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE)
                    {
                        ...
                        memcpy(buffersToSend[currentSendingBufferIdx].currentBuffer, &storeBuffers[alertStoreBufferIdx].currentBuffer[sampleIdxDiff], sizeof(int16_t) * SEND_BUFFER_SIZE);
                        memcpy(buffersToSend[currentSendingBufferIdx].voltageBuffer, &storeBuffers[alertStoreBufferIdx].voltageBuffer[sampleIdxDiff], sizeof(int16_t) * SEND_BUFFER_SIZE);
                        xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (DEBUG_PRINTS)
                    {
                        Serial.printf("[Data request] Negative difference, using two buffers to send data.\n[Alert idx calculation] Alert Idx = %d, sampleIdxDiff = %d, previous buff start idx = %d.\n",alertEventIdx, sampleIdxDiff, prevStartIdx);
                    ...
                    if(xSemaphoreTake(storeBufferMutex[prevAlertStoreBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE)
                    {
                        if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE)
                        {
                            ...
                            memcpy(&buffersToSend[currentSendingBufferIdx].currentBuffer[0], &storeBuffers[prevAlertStoreBufferIdx].currentBuffer[prevStartIdx], sizeof(int16_t) * prevSize);
                            memcpy(&buffersToSend[currentSendingBufferIdx].voltageBuffer[0], &storeBuffers[prevAlertStoreBufferIdx].voltageBuffer[prevStartIdx], sizeof(int16_t) * prevSize);
                            Serial.printf("[Alert IDX calc] Kopiramo sa indeksa %d ovoliko el: %d zadnja vrijednost: %d .\n",prevStartIdx, prevSize, storeBuffers[prevAlertStoreBufferIdx].voltageBuffer[prevStartIdx]);
                            xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
                        }
                        else
                        {
                    ...
                    if(xSemaphoreTake(sendBufferMutex[currentSendingBufferIdx], pdMS_TO_TICKS(1)) == pdTRUE)
                    {
                        memcpy(&buffersToSend[currentSendingBufferIdx].currentBuffer[prevSize], &storeBuffers[alertStoreBufferIdx].currentBuffer[0], sizeof(int16_t) * (alertEventIdx + 1));
                        memcpy(&buffersToSend[currentSendingBufferIdx].voltageBuffer[prevSize], &storeBuffers[alertStoreBufferIdx].voltageBuffer[0], sizeof(int16_t) * (alertEventIdx + 1));
                        xSemaphoreGive(sendBufferMutex[currentSendingBufferIdx]);
                    }
                    else
                    {
...
void setup() {
    ...
    Serial.println("[Setup] BLE alert data Ready");
}
